...
// Helper function to create a simple test image (base64)
...
const testImageUpload = async () => {
  try {
    ...
    const testImageBuffer = createTestImageBase64();
...
const testPaymentProofUpload = async () => {
  try {
    ...
    // Payment data
    formData.append('courseId', '68fce24daec687354cf21555'); // Use the course ID from previous test
    formData.append('studentName', 'Test Student');
    formData.append('studentPhone', '01000000000');
...
const testGroupCreation = async () => {
  try {
    ...
    formData.append('description', 'This is a test group to verify GridFS image upload functionality');
    formData.append('grade', '10');
    formData.append('subject', 'Mathematics');
    formData.append('maxStudents', '30');
    formData.append('isActive', 'true');
    formData.append('coverImage', ''); // Empty cover image
    ...
    const response = await axios.post(`${API_BASE}/groups/admin/groups`, formData, {
      headers: {
        'Authorization': `Bearer ${authToken}`,
        ...formData.getHeaders()
      },
    ...
    if (response.data.success && response.data.data?.coverImage) {
      const imageUrl = response.data.data.coverImage;
      ...
      throw new Error('Group creation response missing success or coverImage');
    }
